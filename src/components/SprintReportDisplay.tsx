        ...
        <Paper elevation={3} sx={{ padding: '2rem', marginTop: '2rem' }}>
          ...
          {(() => {
            ...
            const issuesAddedCount = [...completedIssues, ...incompleteIssues]
              .filter(issue => !!issueKeysAddedDuringSprint[issue.key])
              .length;
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Tooltip title="Copy presentation summary to clipboard">
                          <IconButton 
                            size="small" 
                            onClick={() => {
                              // Get issue keys added during sprint (excluding punted issues)
                              const issueKeysAdded = [...completedIssues, ...incompleteIssues]
                                .filter(issue => !!issueKeysAddedDuringSprint[issue.key])
                                .map(issue => issue.key);
